...
class HuggingFaceDataset(BaseDataset):
    ...
    def _auto_detect_format(self):
        ...
        label_candidates = ["label", "labels", "valence", "arousal", "dominance", "emotion", "sentiment"]
    ...
    def get_dataset_info(self) -> Dict[str, Any]:
        ...
        if self.task_type == "emotion-detection":
            valence_values = [item.get("valence", 0.0) for item in self.data if "valence" in item]
            arousal_values = [item.get("arousal", 0.0) for item in self.data if "arousal" in item]
            dominance_values = [item.get("dominance", 0.0) for item in self.data if "dominance" in item]
            
            if valence_values:
                info["vad_statistics"] = {
                    "avg_valence": sum(valence_values) / len(valence_values),
                    "avg_arousal": sum(arousal_values) / len(arousal_values) if arousal_values else 0,
                    "avg_dominance": sum(dominance_values) / len(dominance_values) if dominance_values else 0
                }
