...
class DummyModelAdapter(BaseModelAdapter):
    ...
    def run(self, inputs: Any) -> Any:
        ...
        if isinstance(inputs, (list, tuple)) and len(inputs) > 0:
            ...
            return [f"raw_prediction_{i}_{random.randint(1, 100)}" for i in range(len(inputs))]
        else:
            # Single prediction
    ...
    def postprocess_output(self, model_output: Any) -> Any:
        ...
        if isinstance(model_output, (list, tuple)):
            ...
            return [f"prediction_{i}_{random.randint(1, 100)}" for i in range(len(model_output))]
        else:
            # Single prediction
