...
class TensorFlowLiteAdapter(BaseModelAdapter):
        ...
        try:
            ...
            if "tokenizer_path" in config:
                try:
                    ...
                    print(
                        "  Warning: transformers not available, using basic tokenization"
                    )
                    self.tokenizer = None
    ...
    def _preprocess_text(self, raw_input: Union[str, Dict[str, Any]]) -> np.ndarray:
        ...
        if self.tokenizer:
            ...
            max_len = (
                self.input_shape[1] if len(self.input_shape) > 1 else self.max_length
            )
        ...
        try:
            ...
            if os.path.exists(image_path):
                image = tf.keras.preprocessing.image.load_img(
                    image_path, target_size=(self.input_shape[1], self.input_shape[2])
                )
