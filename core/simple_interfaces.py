...
class TFLiteModelAdapter(ModelBaseAdapter):
    ...
    def process(self, txt: str) -> Any:
        ...
        try:
            ...
            print(
                "TensorFlow not available. Please install tensorflow: pip install tensorflow"
            )
            return None
...
class HuggingFaceModelAdapter(ModelBaseAdapter):
    ...
    def process(self, txt: str) -> Any:
        ...
        try:
            ...
            print(
                "Transformers not available. Please install transformers: pip install transformers torch"
            )
            return None
...
class SimpleBenchmarkEngine:
    ...
    def _calculate_accuracy(self, predictions: List[Any], samples: List[str]) -> float:
        ...
        base_accuracy = (
            len(valid_predictions) / len(predictions) if predictions else 0.0
        )
        ...
        for model in self.models_under_consideration():
            ...
            for dataset in datasets:
                ...
                if "error" in result:
                    ...
                    print(
                        f"   {dataset['name']}: {accuracy:.3f} ({valid}/{total} valid) {status}"
                    )
