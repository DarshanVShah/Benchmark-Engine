...
class BenchmarkReporter:
    ...
    def generate_comprehensive_report(
        ...
        assessment = self._generate_assessment(
            accuracy, performance_metrics, dataset_info
        )
            ...
            for metric_name, metric_values in results["metrics"].items():
                if isinstance(metric_values, dict):
                    if "accuracy" in metric_values:
                        accuracy_info["per_metric_accuracy"][metric_name] = (
                            metric_values["accuracy"]
                        )

                        # Track overall accuracy
                        if accuracy_info["overall_accuracy"] is None:
                            accuracy_info["overall_accuracy"] = metric_values[
                                "accuracy"
                            ]
                        else:
                            # For multiple metrics, use average
                            current = accuracy_info["overall_accuracy"]
                            accuracy_info["overall_accuracy"] = (
                                current + metric_values["accuracy"]
                            ) / 2
    ...
    def _generate_assessment(
        ...
        if performance["throughput"] is not None:
            ...
            assessment["key_findings"].append(
                f"Model processes {throughput:.1f} samples/second"
            )

        # Overall status
        if assessment["accuracy_grade"].startswith("A") and assessment[
            "performance_grade"
        ].startswith("A"):
            assessment["overall_status"] = "EXCELLENT"
        elif assessment["accuracy_grade"].startswith("B") and assessment[
            "performance_grade"
        ].startswith("B"):
            assessment["overall_status"] = "GOOD"
        elif assessment["accuracy_grade"].startswith("C") and assessment[
            "performance_grade"
        ].startswith("C"):
        ...
        if assessment["overall_status"] == "NEEDS_IMPROVEMENT":
            assessment["recommendations"].append(
                "Consider model optimization or architecture changes"
            )
            assessment["recommendations"].append(
    ...
    def export_summary_report(self, filename: Optional[str] = None) -> str:
            ...
            summary["benchmark_summary"].append(
                {
                    ...
                    "accuracy": result["extracted_metrics"]["accuracy"][
                        "overall_accuracy"
                    ],
                    "performance_grade": result["administrator_assessment"][
                        "performance_grade"
                    ],
                }
            )
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(
                summary, f, indent=2, ensure_ascii=False, cls=BenchmarkJSONEncoder
            )
    ...
    def print_administrator_report(self, report: Dict[str, Any]):
        ...
        print(
            f"Performance Grade: {report['administrator_assessment']['performance_grade']}"
        )
