...
class GenericMetric(BaseMetric):
    ...
    def _calculate_classification_metrics(
        ...
        if len(set(target_classes)) == 2:  # Binary classification
            tp = sum(
                1 for p, t in zip(pred_classes, target_classes) if p == 1 and t == 1
            )
            fp = sum(
                1 for p, t in zip(pred_classes, target_classes) if p == 1 and t == 0
            )
            fn = sum(
                1 for p, t in zip(pred_classes, target_classes) if p == 0 and t == 1
            )
            tn = sum(
                1 for p, t in zip(pred_classes, target_classes) if p == 0 and t == 0
            )
            ...
            return {"accuracy": accuracy, "num_samples": len(predictions)}
